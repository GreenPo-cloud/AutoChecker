...
def _current_B2B_weekly_file(statistics_dir: Path) -> Path:
    """Reuse the newest weekly file until its creation age reaches seven days."""
    pattern = re.compile(r"^\d{2}\.\d{2}\.\d{4}(?:_\d+)?\.txt$")
    candidates = [
        path for path in statistics_dir.iterdir()
        if path.is_file() and pattern.fullmatch(path.name)
    ]
    if candidates:
        newest = max(candidates, key=lambda path: path.stat().st_ctime)
        age = time.time() - newest.stat().st_ctime
        if age < B2B_WEEK_DAYS * 24 * 60 * 60:
            return newest

    base = datetime.date.today().strftime("%d.%m.%Y")
    path = statistics_dir / f"{base}.txt"
    suffix = 2
    while path.exists():
        path = statistics_dir / f"{base}_{suffix}.txt"
        suffix += 1
    path.touch()
    return path


def activate_B2B_order(worker: dict, order_id: str, pdf_name: str) -> tuple[bool, str | None]:
    """Select the order's weekly file, registering a new order when necessary."""
    ...
    with _locked_statistics_file(index_path) as file:
        ...
        for line in lines:
            ...
            stat_file = worker["STATISTICS"] / parts[2]
            if not stat_file.exists():
                return False, "statistics_missing"
            worker["ACTIVE_STAT_FILE"] = stat_file
            worker["B2B_PDF_NAME"] = pdf_name
            return True, None

        stat_file = _current_B2B_weekly_file(worker["STATISTICS"])
        started = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        file.seek(0, 2)
        file.write(f"{normalized_id} | {started} | {stat_file.name}\n")
        file.flush()
        worker["ACTIVE_STAT_FILE"] = stat_file
        worker["B2B_PDF_NAME"] = pdf_name
        return True, None
...
def find_B2B_pdf(pdf_reference: str) -> Path | None:
    """Find Downloads/<Scan value>.pdf using a case-insensitive exact name."""
    ...
    expected_names = {
        (value if value.lower().endswith(".pdf") else value + ".pdf").casefold()
...
def parse_orders_B2B(worker: dict, config: dict, pdf_reference: str) -> dict:
    """Parse one B2B order PDF selected by Scan:#<name>."""
    pdf_path = find_B2B_pdf(pdf_reference)
    if pdf_path is None:
                ...
                if code:
                    if identity.department.upper() == "B2B" and code.startswith("#"):
                        ...
                        if is_cancelled:
                            ...
                            pdf_path = find_B2B_pdf(code)
                            if pdf_path is None:
                                performance_stats.record_scan()
                                performance_stats.record_mistake()
                                message = f"XXX B2B PDF not found: {code}"
                                Printer(message)
                                send(worker["DISPLAY"], message)
                            else:
                                activated, reason = activate_B2B_order(
                                    worker, code, pdf_path.name
                                )
                                if not activated:
                                    performance_stats.record_scan()
                                    performance_stats.record_mistake()
                                    if reason == "cancelled":
                                        ...
                                        message = f"XXX Statistics file missing for {code}"
                                    else:
                                        message = f"XXX Invalid B2B index entry for {code}"
                                    Printer(message)
                                    send(worker["DISPLAY"], message)
                                else:
                                    loaded_orders = parse_orders_B2B(worker, config, code)
                                    if loaded_orders:
                                        orders = loaded_orders
